...
public class TestEquilibrio {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i<N-2; i++){ //righe tranne l'ultima
            ...
            w -= somma_controllo_positivi;
            revers += somma_controllo_negativi;
            for(int j=i+1; j!=N; j++){ // itero per un numero di volte pari alle posizioni disponibili
                ...
                if(mat[i][pos] == 0){
                    if(caselle_positive < (N-1)/2){
                        mat[i][pos] = mat[pos][i] = rand.nextInt(( w - (num-2)/2 ) -1  ) +1;
                        w -= mat[i][pos];
                        revers += mat[i][pos];
                        caselle_positive++;
                        num -= 2;
                        ...
                        if (caselle_negative == 1){       //regola l'ultima cella della riga
                            mat[i][pos] = mat[pos][i]= -revers;
                        }else {
                                mat[i][pos] = mat[pos][i]=  -rand.nextInt(revers - (caselle_negative - 1)) - 1; //revers é la quantitá positiva somma delle casellle giá assegnate in quella riga;
                                revers += mat[i][pos];
                                caselle_negative -= 1;
                             }
                    }
